...
**tigeryst** is my personal portfolio site. Because this is the `tigeryst/tigeryst` profile repo, `README.md` is my GitHub profile page, not app documentation: setup and workflow live in [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
There is no test runner in this project. Verify with `npm run format:check && npm run lint && npm run typecheck && npm run build` before claiming done.
